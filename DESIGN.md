...
> Rankly is a lightweight SEO audit workspace for deployed marketing sites, portfolios, and Next.js apps. It scans public routes for titles, meta descriptions, Open Graph and Twitter tags, canonical URLs, robots directives, sitemap coverage, and JSON-LD, then turns the results into page-level findings and fix history so a solo builder or small team can tighten search visibility without redesigning the product.
...
- **Mood:** crisp, precise, spare, actionable
...
- The dashboard + the primary workflow ("Add a deployed site and run its first audit to see the full SEO report.") get the most polish.
...
- HERO: STATEMENT / MANIFESTO — a single bold product sentence fills the first frame, CTA beneath; the visual is deferred to a later section.
- NAV: CENTERED WORDMARK — links left, wordmark centered, CTA right (a balanced classic bar).
- SECTION ORDER STRATEGY: STORY / NARRATIVE — order the sections as a story (setup -> turn -> payoff); a sticky-scroll step (StickyProtocol) is OPTIONAL, only if time allows.
- SIGNATURE MOVE (the ONE bold compositional move, used once): A dramatic FULL-BLEED SCALE JUMP between two adjacent sections.
